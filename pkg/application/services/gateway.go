...
func (s *GatewayService) GetNetworkState(ctx context.Context, gateway *entities.Gateway) error {
	...
	token := s.pool.GetClient(gateway.GetOptions().ClientID).Publish(gateway.GetCommandTopic(), 0, false, message)
...
func (s *GatewayService) RemoveDevice(gateway *entities.Gateway, deviceId string) mqtt.Token {
	...
	return s.pool.GetClient(gateway.GetOptions().ClientID).Publish(gateway.GetCommandTopic(), 0, false, message)
}
